...
void render() {
  if (!exploded) {
    ...
    noStroke();
    fill(255, 255, 0);
    ellipse(x, y, 20, 20); // Increase the size of the ellipse for the enemy bomb
  } else {
    // explosion
    noStroke();
    int fadeTime = 500;
    ...
    ellipse(x, y, 30, 30); // The size of the explosion remains unchanged
  }
}
}
...
void draw() {
  if (start) {
    ...
    for (int j = 0; j < 6; j++) {
      ...
      float cityY = ground - 0; // position the cities exactly on the red line
    ...
    for (int i = 0; i < bombs.length; i++) {
      if (bombs[i] != null) {
        ...
        if (ABM != null && ABM.collidesWithBomb(bombs[i])) {
          ABM.explode();
          bombs[i].exploded = true;
        }
      }
